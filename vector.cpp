...
Vector::Vector(int dim, float def) //Êîíñòðóêòîð (ðàçìåðíîñòü, çíà÷åíèå ïî óìîë÷àíèþ)
{
	dimension = dim;
	data = new float[dim];
	for (int i = 0; i < dim; i++)
...
Vector::~Vector() // Äåñòðóêòîð (ìåòîä, êîòîðûé âûçûâàåòñÿ ïåðåä óäàëåíèåì îáúåêòà)
{
...
float& Vector::operator[](int i) // Îïåðàòîð äîñòóïà ê äàííûì
{
	return data[i];
}
